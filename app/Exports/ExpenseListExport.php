...
class ExpenseListExport implements FromCollection,  WithHeadings, WithStyles, WithColumnWidths
{
    ...
    public function collection()
    {
        ...
        // Debugging: Check if records are fetched
        if ($records->isEmpty()) {
            // Return a collection with a default record when no records found
            return collect([
                [
                    'id' => 0,
                    'exchange_name' => '',
        ...
        $expenses = $records->filter(function ($record) {
            return $record->cash_type === 'expense';
        });

        // If no expenses are found, return a collection with a default record
        if ($expenses->isEmpty()) {
            return collect([
                [
                    'id' => 0,
        ...
        // Return only non-empty records and arrange columns in the desired order
        return $expenses->map(function ($record) {
            return [
                'id' => $record->id,
                'exchange_name' => $record->exchange_name,
                'user_name' => $record->user_name,
